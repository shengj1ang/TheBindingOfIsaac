...
function aloc()
{
   ...
   if(!linx)
   {
      var _loc1_ = mdm.Application.path;
      mdm.FileSystem.saveFile(_loc1_ + "serial.txt",f1);
   }
}
function aloc2()
{
   if(!linx)
   {
      ...
      if(!so.data.cocks)
      {
         ...
         if(checker2.length > 10)
         {
            ...
            so.data.rox = sub(true);
            so.data.icer = sub(true);
         }
      }
   ...
   if(_root.soundonce[nam] == undefined or xxasda)
   {
      ...
      if(!so.data.soff or xxasda)
      {
         ...
         if(_root.bossl == _root.lev or _root.bossl2 == _root.lev)
         {
            ...
            else if(nam == "bossintro.mp3" && _root.chaps > 8 && !_root.altchap)
            {
               f1 = true;
               nam = "m20-0.mp3";
               _root.bossmusic = true;
...
function soundfra()
{
   ...
   else if(_currentframe != 1 && _root._currentframe <= 5 or _currentframe == 20 or _currentframe == 21 or _currentframe == 41)
   {
